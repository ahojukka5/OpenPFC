...
public:
  ...
  WavePacketSample sample(RealField &u) {
    ...
    MPI_Allreduce(&local_n_ll, &global_n, 1, MPI_UNSIGNED_LONG_LONG, MPI_SUM,
                  m_comm);
    ...
    result.mode_amplitude =
        (den > 0.0) ? std::sqrt(global_c * global_c + global_s * global_s) / den
                    : 0.0;
    ...
    m_u2.for_each_owned([&](const pfc::Real3 &x, double lp) {
      if (x[0] < edge || x[0] > Lx - edge) {
        const double env2 =
            std::max(0.0, 2.0 * lp - 2.0 * result.mean * result.mean);
        local_edge_peak = std::max(local_edge_peak, env2);
      }
    });
    double global_edge_peak = 0.0;
    MPI_Allreduce(&local_edge_peak, &global_edge_peak, 1, MPI_DOUBLE, MPI_MAX,
                  m_comm);
...
public:
  explicit PulseDiagnostics(MPI_Comm comm, double window)
      : m_comm(comm), m_window(window) {}

  template <class RealField> PulseSample sample(RealField &u) {
    PulseSample result;
    const auto reduced = pfc::sim::reduce_owned(u, m_comm);
    result.mean =
        reduced.count == 0 ? 0.0 : reduced.sum / static_cast<double>(reduced.count);
    ...
    MPI_Allreduce(&local_tail_sumsq, &global_tail_sumsq, 1, MPI_DOUBLE, MPI_SUM,
                  m_comm);
