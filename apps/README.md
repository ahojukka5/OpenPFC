...
| [**`inverse_homogenization/`**](inverse_homogenization/README.md) | **Periodic inverse homogenization** (target \(C_H\); issue #161). Explicit sixteenth-app exception: PDE-constrained inverse design, not another PDE demo. Forward \(C_H\), adjoint, Allen–Cahn inverse loop, CH process family, manufacturability, and 3-D cells are in tree. Catalog stays closed after this. | CLI (`--key=value`) | Reuses `openpfc/solvers/microelasticity/microelasticity.hpp` (six imposed-strain FFT solves); CPU + optional HIP Green |
