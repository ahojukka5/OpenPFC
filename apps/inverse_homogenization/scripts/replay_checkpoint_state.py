...
"""CPU/HIP restart qualification: moving fields and a restart inside a quiet hold."""
import argparse
import csv
import hashlib
import json
...
def main():
    ...
    p.add_argument('--cpu', action='store_true', help='qualify the CPU entrypoint without HIP snapshot ledgers')
    args = p.parse_args()
    args.output.mkdir(exist_ok=False)
    result = {'binary_sha256': sha(args.binary), 'driver_sha256': sha(Path(__file__)),
              'target_sha256': sha(args.target), 'ranks': args.ranks, 'implementation': 'cpu' if args.cpu else 'hip', 'cases': []}
    ...
    for name, init, volume, maximum, cut in [('hold', 'uniform', 1, 500, 50),
        ...
        if not args.cpu:
            case_args.append(f'--run-id={name}')
        ...
        result['cases'].append({'name': name, 'cut': cut, 'saved_state': saved,
                                ...
                                'ledger_failures_rejected': [] if args.cpu else ['missing', 'truncated', 'malformed'],
