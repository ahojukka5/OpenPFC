...
/**
 * @brief Periodic signed wavenumber \(k = 2\pi i / (N \Delta)\) with wrap at \(N/2\).
 *
 * @param i  Mode index on `[0, N)`.
 * @param n  Global point count on that axis.
 * @param f  \(2\pi / (N \Delta)\).
 */
[[nodiscard]] inline double signed_wavenumber(int i, int n, double f) noexcept {
  return (i <= n / 2) ? static_cast<double>(i) * f : static_cast<double>(i - n) * f;
}

[[nodiscard]] inline std::size_t outbox_volume(const pfc::Box3i &outbox) {
  const auto axis_count = [](int lo, int hi) -> std::size_t {
    return static_cast<std::size_t>(hi - lo + 1);
  };
  return axis_count(outbox.low[0], outbox.high[0]) *
         axis_count(outbox.low[1], outbox.high[1]) *
         axis_count(outbox.low[2], outbox.high[2]);
}

/**
 * @brief Fourier symbol of \(\nabla^2\): \(k_\mathrm{lap}=-(k_x^2+k_y^2+k_z^2)\).
 *
 * @details Host-only and FFT-backend-free. Signed wavenumbers wrap at \(N/2\).
 *          Layout is k-outer, i-inner, matching HeFFTe's local outbox walk.
 *          The implicit-Euler table is this symbol mapped to
 *          \(1/(1 - \Delta t\, D\, k_\mathrm{lap})\).
 *
 * @param k_lap    Destination, length equal to the outbox volume.
 ...
 * @throws std::invalid_argument when `k_lap.size()` does not match the outbox
 *         volume implied by `outbox.low`/`outbox.high`.
 */
inline void fill_laplacian_symbol(std::span<double> k_lap, const pfc::Box3i &outbox,
                                  pfc::Int3 size, pfc::Real3 spacing) {
  const std::size_t expected = outbox_volume(outbox);
  if (k_lap.size() != expected) {
    throw std::invalid_argument("fill_laplacian_symbol: k_lap size " +
                                std::to_string(k_lap.size()) + " != outbox volume " +
  ...
  for (int k = outbox.low[2]; k <= outbox.high[2]; ++k) {
    for (int j = outbox.low[1]; j <= outbox.high[1]; ++j) {
      for (int i = outbox.low[0]; i <= outbox.high[0]; ++i) {
        const double ki = signed_wavenumber(i, size[0], fx);
        const double kj = signed_wavenumber(j, size[1], fy);
        const double kk = signed_wavenumber(k, size[2], fz);
        k_lap[idx++] = -(ki * ki + kj * kj + kk * kk);
      }
    }
  }
}
 ...
 *          device by `SpectralHeatPropagatorHIP`. Fills \(k_\mathrm{lap}\) then
 *          maps each entry to \(1/(1 - \Delta t\, D\, k_\mathrm{lap})\).
 *
...
inline void fill_implicit_euler_symbol(std::span<double> opL,
  ...
  fill_laplacian_symbol(opL, outbox, size, spacing);
  for (double &v : opL) {
    v = 1.0 / (1.0 - dt * D * v);
  }
}
