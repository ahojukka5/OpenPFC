...
- **[`src/hip/heat3d_fd_hip.cpp`](src/hip/heat3d_fd_hip.cpp)** — HIP twin of compact FD. `FDGPUStack<HIPSpace>` owns the padded device field and `HaloExchange<HIPSpace>`. Default step (`HEAT3D_HALO_OVERLAP=1`) launches the halo-independent interior on a non-blocking compute stream, posts Faces MPI on the default stream, then `finish()` + one-launch boundary shell. `=0` is blocking `halo.exchange()` / full RHS; `=2` also pumps `MPI_Testall`. Built when `OpenPFC_ENABLE_HIP` is on.
