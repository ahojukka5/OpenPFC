...
- **[`src/hip/heat3d_fd_hip.cpp`](src/hip/heat3d_fd_hip.cpp)** — HIP twin of compact FD. `FDGPUStack<HIPSpace>` owns the padded device field and `HaloExchange<HIPSpace>`. Default step on more than one rank (`HEAT3D_HALO_OVERLAP=1`) launches the halo-independent interior on a non-blocking compute stream, posts Faces MPI on the default stream, then `finish()` + one-launch boundary shell. A single rank has no MPI halo, so the default falls back to blocking; `=0` forces blocking; `=2` also pumps `MPI_Testall`. Built when `OpenPFC_ENABLE_HIP` is on.
