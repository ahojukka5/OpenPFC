...
- **[`src/hip/heat3d_fd_hip.cpp`](src/hip/heat3d_fd_hip.cpp)** — HIP twin of compact FD. `FDGPUStack<HIPSpace>` owns the padded device field and `HaloExchange<HIPSpace>`. Default step is blocking `halo.exchange()` / `heat3d::fd_rhs_hip` / `heat3d::euler_axpy_hip`. `HEAT3D_HALO_OVERLAP=1` launches the halo-independent interior on a non-blocking compute stream, posts Faces MPI on the default stream, then `finish()` + one-launch boundary shell (`=2` also pumps `MPI_Testall`). Default stays blocking until a clean production A/B justifies otherwise. Built when `OpenPFC_ENABLE_HIP` is on.
