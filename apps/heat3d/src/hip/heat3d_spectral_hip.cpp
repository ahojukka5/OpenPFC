 ...
 * Rank 0 prints `HEAT3D_SPECTRAL_HIP_PHASE` medians for forward FFT,
 * k-space multiply, and inverse FFT (hipEvent, after warmup).
 */
...
int run_heat3d_spectral_hip(const heat3d::RunConfig &cfg, int rank, int nproc) {
  ...
  std::vector<double> phase_fwd;
  std::vector<double> phase_mul;
  std::vector<double> phase_bwd;
  std::vector<double> phase_wall_ms;
  phase_fwd.reserve(static_cast<std::size_t>(std::max(0, cfg.n_steps - warmup)));
  phase_mul.reserve(phase_fwd.capacity());
  phase_bwd.reserve(phase_fwd.capacity());
  phase_wall_ms.reserve(phase_fwd.capacity());

  MPI_Barrier(MPI_COMM_WORLD);
  const double t_start = MPI_Wtime();
  for (int step = 0; step < cfg.n_steps; ++step) {
    const bool record = step >= warmup;
    if (record && prof) {
    ...
    if (record) {
      const auto ph = prop.phase_times_ms();
      phase_fwd.push_back(static_cast<double>(ph.forward_ms));
      phase_mul.push_back(static_cast<double>(ph.multiply_ms));
      phase_bwd.push_back(static_cast<double>(ph.backward_ms));
      phase_wall_ms.push_back(wall * 1.0e3);
      if (prof) {
  ...
  if (rank == 0 && !phase_fwd.empty()) {
    const double fwd = median_of(phase_fwd);
    const double mul = median_of(phase_mul);
    const double bwd = median_of(phase_bwd);
    const double wall_ms = median_of(phase_wall_ms);
    const double sum = fwd + mul + bwd;
    const auto pct = [sum](double x) { return sum > 0.0 ? 100.0 * x / sum : 0.0; };
    const auto pct_wall = [wall_ms](double x) {
      return wall_ms > 0.0 ? 100.0 * x / wall_ms : 0.0;
    };
    std::cout << std::setprecision(4);
    std::cout << "HEAT3D_SPECTRAL_HIP_PHASE n=" << phase_fwd.size()
              << " median_ms forward=" << fwd << " multiply=" << mul
              << " backward=" << bwd << " wall_step=" << wall_ms << "\n";
    std::cout << "HEAT3D_SPECTRAL_HIP_PHASE pct_of_phases forward=" << pct(fwd)
              << " multiply=" << pct(mul) << " backward=" << pct(bwd) << "\n";
    std::cout << "HEAT3D_SPECTRAL_HIP_PHASE pct_of_wall_step forward="
              << pct_wall(fwd) << " multiply=" << pct_wall(mul)
              << " backward=" << pct_wall(bwd) << "\n";
  }
