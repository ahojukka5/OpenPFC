...
def test_submit_dry_run_768(tmp_path):
    ...
    assert "HEAT3D_PROTOCOLS=p2p_plined:p2p:alltoallv:alltoall" in proc.stdout_text
...
def test_batch_refuses_wrong_or_missing_account(account):
    env = os.environ.copy()
    env.update(SLURM_JOB_ACCOUNT=account, HEAT3D_SPECTRAL_HIP_BIN="/bin/true",
               HEAT3D_NX="512", HEAT3D_NY="512", HEAT3D_NZ="512")
    batch = ROOT / "docs/lumi_slurm/heffte_protocol_tournament.sbatch"
