...
step (`HIPSpectralStack` + `1/(1-Δt D k_lap)`). Same CLI as the CPU
spectral driver (`<N> <n_steps> <dt>`). I/O off via `HEAT3D_PROFILE_JSON`.
Same bind/visibility as `tungsten_hip` / `heat3d_fd_hip`. Compare FFT time
to the FD HIP halo+stencil on the **same PDE**; do not claim “FD is faster
than tungsten.”

Submit with `submit_heat3d_spectral_hip_scaling.sh`. Pins are not in-tree
until a 1-GCD-busy grid is sized and the 8→16→24 (or 32) curve is recorded.
