...
REQUIRED_ACCOUNT="project_462001245"
FORBIDDEN_ACCOUNT="project_462001519"
ACCOUNT="${ACCOUNT:-${REQUIRED_ACCOUNT}}"
...
if [[ "${ACCOUNT}" == "${FORBIDDEN_ACCOUNT}" ]]; then
  echo "refusing ACCOUNT=${ACCOUNT}; issue #61 bills ${REQUIRED_ACCOUNT}" >&2
  exit 2
fi
if [[ "${ACCOUNT}" != "${REQUIRED_ACCOUNT}" ]]; then
  echo "refusing ACCOUNT=${ACCOUNT}; want ${REQUIRED_ACCOUNT}" >&2
  exit 2
fi
