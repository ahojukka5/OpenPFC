...
Frozen interval for a scalar R: t in [T_AVG_LO, T_AVG_HI] = [0.10, 0.70],
provided the X/O pair stays well-conditioned. Peak R is secondary.
"""
...
def attach_geometry(directory, n, eta, flux_rep):
    ...
    Rs = [r["R"] for r in interior if r.get("R") is not None]
    Ss = [r["S_local"] for r in interior if r.get("S_local") is not None]
    flux_rep["scaling"] = {
        "T_AVG_LO": T_AVG_LO,
        "T_AVG_HI": T_AVG_HI,
        "n_avg": len(interior),
        ...
        "delta_mean": float(np.mean([r["delta"] for r in interior])),
        "L_mean": float(np.mean([r["L"] for r in interior])),
        "B_up_mean": float(np.mean([r["B_up"] for r in interior])),
        "aspect_mean": float(np.mean([r["aspect"] for r in interior
...
def print_geo(rep, tag=""):
    ...
    if "error" in rep or "scaling" not in rep:
        return
    s = rep["scaling"]
    print("  geometry means t in [%.2f, %.2f]: delta=%.4g L=%.4g L/delta=%.3g "
          "B_up=%.4g S_local=%.4g R_mean=%.4e ± %.1e" % (
              s["T_AVG_LO"], s["T_AVG_HI"], s["delta_mean"], s["L_mean"],
    ...
    for r in rows[:: max(1, len(rows) // 8)]:
        print("  t=%.4f delta=%.4g L=%.4g B_up=%.4g S=%.4g R=%.4e" % (
