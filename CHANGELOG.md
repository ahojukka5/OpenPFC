...
- Directional-solidification `FixedBC` and `MovingBC` live in `tungsten::`
  ...
  those types.
- The installed library propagates HeFFTe, so a spectral consumer links
  only `OpenPFC::openpfc`. Public spectral headers still include HeFFTe.
...
- **HeFFTe topology-crossover campaign tooling** (issue
  ...
  placement metadata collection. All three 128-node allocations elect
  `p2p_plined` (H3 rejected there; H2 not supported). Does not change
  the production reshape default.
- **Pre-execution HeFFTe reshape descriptors and harvest status**
  ...
  keeps rejected and missing protocol rows. Status spread is the
...
- **FD-order geometry table and harvest status** (issue
  ...
  and `EvenCentralD2` stencils, plus a harvest command that keeps
  rejected runs.
...
- **Heat3D spectral HIP `HEAT3D_USE_REORDER` / `HEAT3D_GPU_AWARE`**
  (issue #51). Explicit `0`/`1` overrides after the production overlay.
  Unset keeps rocFFT `use_reorder` and GPU-aware MPI.
...
- **Heat3D HIP overlap** uses `pfc::comm::GpuHaloOverlap` for the
  production two-stream step. `HEAT3D_HALO_OVERLAP` and diag timing stay
  in the app (issue #68).
...
- **3-D re-entrant honeycomb forward oracle**
  ...
  inverted honeycomb with Gibson--Robert four-fold vertical patterning.
  Prescribed struts, not an extrusion and not inverse design.
...
- **Alloy-dendrite 3-D 8-GCD showcase** (issue #12). Spherical seed,
  ...
  `apps/alloy_dendrite_elastic/slurm/showcase_dendrite3d.sbatch`.
...
- **Alloy-dendrite 8-GCD showcase** (issue #11).
  ...
  gathers 2-D tip diagnostics at any rank count. HIP defaults match the
  CPU Al-4.5wt%Cu arm (`lambda=D_l/a2`, `lambda_el=lambda` when
  `--elastic=1`). Recipe:
