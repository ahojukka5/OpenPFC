 ...
 *
 * `SnapshotSeries` and `DiagnosticsSeries` stay independent. Pass them
 * from the save observer. A checkpoint belongs in `set_checkpoint_hook`:
 * that hook runs only for an accepted state that `do_save()` accepts.
 ...
 * and still does not save or checkpoint.
 *
 ...
 * ran. It does not frame `prepare_stage`.
 */
...
public:
   ...
   *
   * Does not call the save observer, the checkpoint hook, or the profiler,
   * and does not change `Time`.
   */
  ...
  /**
   * @brief Accepted-save hook. Use this for `CheckpointService::save`.
   *
   * Not called for a rejected attempt, and not called when `do_save()`
   * is false.
   */
  void set_checkpoint_hook(std::function<void(const Time &)> hook) {
    m_checkpoint = std::move(hook);
  }
  ...
  template <class Step> void run(Step &&step) {
    ...
    auto on_save = [this](const Time &now) { deliver_save(now); };
  ...
  void run_attempts(Step &&step, int max_consecutive_rejections = 10000) {
    ...
    pfc::sim::run_attempts(
        ...
        on_start, apply, on_save, max_consecutive_rejections);
  }
...
private:
  ...
  void deliver_save(const Time &now) {
    if (m_observer) m_observer(now);
    if (m_checkpoint) m_checkpoint(now);
  }
  ...
  std::function<void(const Time &)> m_checkpoint;
