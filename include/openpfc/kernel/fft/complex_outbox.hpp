 ...
 * @brief r2c complex-outbox process-grid selection and HeFFTe plan costs
 *
 * @details
 * HeFFTe `split_world()` assigns approximately equal extents; exact
 * divisibility is not required. OpenPFC's production default still
 * y-slabs the r2c outbox only when `Ny % nproc == 0`. That refuses a
 * legal uneven `1×nproc×1` (and any other `gz=1` grid) and leaves an
 * extra distributed reshape when the real inbox is a z-slab.
 *
 * Experimental selectors, fail-closed:
 *
 * - `OPENPFC_FFT_COMPLEX_PROC_GRID=gx,gy,gz` (or `gx x gy x gz`)
 * - `OPENPFC_FFT_COMPLEX_OUTBOX=min_reshape` picks the legal complex
 *   grid that minimises distributed HeFFTe reshapes, then local-box
 *   imbalance, then aspect ratio
 *
 * Do not encode application sizes such as `N == 1200`.
 */
...
/// OpenPFC r2c on 9+ ranks uses slabs and reordered 1-D FFTs.
[[nodiscard]] inline heffte::plan_options slab_r2c_plan_options() {
  return heffte::plan_options(true, heffte::reshape_algorithm::alltoallv, false);
}
...
[[nodiscard]] inline bool split_boxes_are_legal(
    const std::vector<heffte::box3d<int>> &boxes,
...
plan_r2c_outbox_cost(const heffte::box3d<int> &real_world, const Int3 &real_grid,
  ...
  if (!complex_proc_grid_is_legal(complex_world, complex_grid, nproc)) {
    throw std::invalid_argument(
        ...
        std::to_string(complex_grid[0]) + "x" + std::to_string(complex_grid[1]) +
        "x" + std::to_string(complex_grid[2]));
  }
  ...
  // One all-peer waitany round per MPI reshape per direction.
...
[[nodiscard]] inline bool complex_outbox_min_reshape_enabled() {
  ...
  throw std::invalid_argument(
      ...
      " must be unset, \"default\", or \"min_reshape\", got \"" + v + "\"");
}

/// Production default: y-slab only when Ny divides nproc exactly.
[[nodiscard]] inline Int3 default_complex_proc_grid_for_r2c(
...
complex_proc_grid_for_r2c(const Int3 &real_grid,
  ...
  if (forced[0] > 0) {
    if (!complex_proc_grid_is_legal(complex_world, forced, nproc)) {
      throw std::invalid_argument(
          std::string(kComplexProcGridEnv) + "=" + std::to_string(forced[0]) +
          "," + std::to_string(forced[1]) + "," + std::to_string(forced[2]) +
          " is not a legal complex process grid for this r2c world");
    }
    return forced;
  }
  if (complex_outbox_min_reshape_enabled()) {
    return select_min_distributed_reshape_complex_grid(
        real_world, real_grid, complex_world, r2c_direction, options);
  }
  return default_complex_proc_grid_for_r2c(real_grid, complex_world,
                                           r2c_direction);
}
